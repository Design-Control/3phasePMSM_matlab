...
a_p = 0.8; %극호비
theta_w = pi/6; %short pitch angle

W_factor = 1; 
%distribution factor(q, theta_d)를 고려하면 1, 고려하지 않으면 0
% W_factor = 0 이면 q, theta_d 의 값은 사용되지 않음. 
q = 3; %number of distributed coils
theta_d = pi/9; %distribution angle
...
B_amp = 1;
B_theta_e = B_amp*square(theta_e); %0.785*sqrt(1)*square(theta_e)%전기각에 대한 자기장
...
K_p = zeros(1, max_harmonics); %극호비에 대한 자기장 감소 계수
for i = 1:max_harmonics %극호비 고려
    K_p(i) = cos(i*pi/2*(1-a_p));
    B_g(i) = B_g(i)*K_p(i);
end
